    ...
    userSettings = {
      ...
      github.copilot.chat = {
        ...
        codeGeneration.instructions = [
          ...
          # 💻 品質・テスト
          { text = "コードには英語コメントと適切なドキュメント、丁寧なエラー処理を含めます。プログラムを修正した際には必ずコンパイルやテストを実行して動作確認します。エラーが発生した場合は、エビデンスと最新のドキュメントに基づいた解決策を段階的に提案します。"; }
        ...
        commitMessageGeneration.instructions = [
          # 📝 コミットメッセージ規約
          { text = "コミットメッセージのタイトル行は GitHub PR タイトルとしても使われることを考慮し、50～72文字以内に収め、内容を端的に伝えるようにします。"; }
        ];
      };
